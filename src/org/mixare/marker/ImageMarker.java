...
/**
 * Local Image Marker that handles drawing images locally (extents
 * {@link org.mixare.marker.LocalMarker LocalMarker})
 * 
 * Note: LinkURL is the url when marker is clicked on. Note: ImageURL is the url
 * that links to solid image.
 * 
 * @author devBinnooh
 * @author A.Egal
 */
public class ImageMarker extends LocalMarker {
	...
	/**
	 * Constructor with the given params. Note Image will be set to default
	 * (empty square). Please Set the image
	 * {@link org.mixare.marker.ImageMarker#setImage(Bitmap)}, or Call this
	 * class with Image URL.
	 * 
	 * @see org.mixare.marker.ImageMarker#ImageMarker(String, String, double,
	 *      double, double, String, int, int, String, String)
	 * @param String
	 *            Marker's id
	 * @param String
	 *            Marker's title
	...
	/**
	 * Constructor with the given params. Marker will handle retrieving the
	 * image from Image URL, Please ensure that it links to an Image.
	 * 
	 * @param String
	 *            Marker's id
	 * @param String
	...
	public ImageMarker(String id, String title, double latitude,
		...
		try {
			...
			this.setImage(BitmapFactory.decodeStream(imageURI.openConnection()
					.getInputStream()));
	 ...
	 * 
	 * @param PaintScreen
	 *            View Screen that title screen will be drawn into
	 */
	public void drawTitle(final PaintScreen dw) {
		if (isVisible) {
			...
			dw.paintText3D(textStr, getURL(), new PointF(getSignMarker().x,
					getSignMarker().y + maxHeight), currentAngle + 90);
		}
	}
